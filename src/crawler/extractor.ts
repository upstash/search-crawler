...
export async function crawlDocumentation(
  ...
  try {
    ...
    if (response.status !== 200) {
      throw new Error(
        chalk.red(`Failed to crawl: ${url} - Status: ${response.status}`)
      );
    }
    ...
    $(
      "script, style, nav, footer, .nav, .sidebar, .menu, .footer, .sidebar-group, a"
    ).remove();

    let body = $("body");
    if (body.length === 0) {
      body = $('div[role="main"], .body[role="main"]');
    }
    const baseTitle = body.find("h1, h2").first().text().trim();
      ...
      headings.each((index, element) => {
        ...
        if (cleanContent) {
          ...
          if (headerParent && headerParent.length > 0) {
            ...
            sectionPath = headingId
              ? `${url.replace(new URL(url).origin, "")}#${headingId}`
