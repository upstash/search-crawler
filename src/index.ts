...
export async function crawlAndIndex(
  ...
  try {
    ...
    if (s)
      s.message(
        chalk.cyan(
          `Crawled ${results.length} content sections. Fetching existing data`
        )
      );
    ...
    if (newContents.length > 0) {
      if (s)
        s.message(
          chalk.cyan(
            `Found ${newContents.length} new records. Upserting to Upstash`
          )
        );
      ...
      if (s)
        s.stop(
          chalk.green(
            `✅ Successfully crawled and upserted ${newContents.length} new records!`
          )
        );
      ...
      if (s) s.stop(chalk.green("No new records found"));
      return {
        success: true,
        newRecordsCount: 0,
        totalRecordsCount: results.length,
